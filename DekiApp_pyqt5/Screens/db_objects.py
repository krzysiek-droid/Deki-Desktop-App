...
class MainConstruction(Construction):
    def __init__(self):
        ...
        self.table_name = f"{self.company_name}_mainConstructions"
    ...
    def save_main_construction(self):
        try:
            if not self.db.is_table(self.table_name):
                ...
                print('Database found, adding...', end=" ")
                self.folderPath = mainConstructions_filepath + fr'\{self.info["serial_number"]}'
                try:
                    pathlib.Path.mkdir(pathlib.Path(self.folderPath))
                    print(f'Directory {self.info["serial_number"]} created. ', end=' ')
                except FileExistsError:
                    print(f'Directory {self.info["serial_number"]} already exists. Trying to save...', end=' ')
                ...
                try:
                    ...
                    print(f'Directory for subConstructions -{subs_path}- already exists. Trying to save...', end=' ')
            ...
            self.db.create_table(f"{self.info['serial_number']}_SubConstructions",
                                 database_settings['subConstructions_columns'])
            self.db.create_table(f"{self.info['serial_number']}_modelWelds", database_settings['modelWelds_columns'])
            # self.db.create_table(f"{self.info['serial_number']}_welds")
            return 1
        except Exception as e:
            print(f"MainConstruction Object save failure -> {e}")
            return 0
    ...
    def load_info(self, construct_id):
        try:
            ...
            self.subConstructions_tableName = f"{self.info['serial_number']}_subConstructions"
            self.subConstructions_df = self.db.table_into_DF(f"{self.info['serial_number']}_SubConstructions")
